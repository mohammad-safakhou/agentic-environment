# Historical small-server pilot

This records the pilot verified on a shared Ubuntu 26.04 host with 2 CPUs and 4 GB RAM. It was removed at the owner's request on 2026-09-25 so they could install the environment themselves. The repeatable one-task setup path is now in [SMALL_SERVER.md](SMALL_SERVER.md). The ports and paths below describe that earlier pilot; they are no longer live.
...
The former Windmill workspace was `agent-environment-pilot`. Its database, credentials and services were deleted with the pilot.
...
The full stack's Homepage, Beszel, Backrest, Tailscale Serve and second worker were intentionally omitted from this low-resource pilot. The repeatable small profile follows the same core shape, while the complete distribution and its remaining acceptance checks are described in [README.md](README.md) and [ACCEPTANCE.md](ACCEPTANCE.md).
